import tw from "twin.macro";

export const Title = tw.h1`text-center text-black font-bold text-xl mb-2`;
export const Main = tw.div`flex items-center justify-center min-h-screen py-8`;
export const MainContent = tw.div`container mx-auto flex items-center justify-center min-h-screen py-8`;
export const WhiteContainer = tw.div`bg-white mt-24 p-8 shadow-lg transform`;
export const LayOutGrid = tw.div`grid grid-cols-1 sm:grid-cols-2 gap-4`;
export const Column = tw.div`col-span-1`;
export const GrayText = tw.div`text-gray-700 bg-white p-4 shadow-md divide-y divide-solid`;
